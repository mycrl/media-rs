use crate::{config::HttpFlv, proto::http::*, router};
...
pub async fn run(cfg: HttpFlv, router: Arc<router::Router>) -> anyhow::Result<()> {
    let app = Router::new()
        .route("/:name", get(fork_socket))
        .layer(
            CorsLayer::new()
                .allow_origin(cfg.allow_origin.as_str().parse::<HeaderValue>().unwrap()),
        )
        .with_state(router)
